import { Component, ViewChild } from '@angular/core';
import { Platform, Nav } from 'ionic-angular';
import { StatusBar } from '@ionic-native/status-bar';
import { SplashScreen } from '@ionic-native/splash-screen';

import { MusikerPage } from '../pages/musiker/musiker';
import { MusikerDetailsPage } from '../pages/musiker-details/musiker-details';
import { InstrumentePage } from '../pages/instrumente/instrumente';
import { InstrumentenDetailsPage } from '../pages/instrumenten-details/instrumenten-details';
import { KalenderPage } from '../pages/kalender/kalender';


import { HomePage } from '../pages/home/home';



@Component({
  templateUrl: 'app.html'
})
export class MyApp {
  @ViewChild(Nav) navCtrl: Nav;
    rootPage:any = HomePage;

  constructor(platform: Platform, statusBar: StatusBar, splashScreen: SplashScreen) {
    platform.ready().then(() => {
      // Okay, so the platform is ready and our plugins are available.
      // Here you can do any higher level native things you might need.
      statusBar.styleDefault();
      splashScreen.hide();
    });
  }
  goToHome(params){
    if (!params) params = {};
    this.navCtrl.setRoot(HomePage);
  }goToMusiker(params){
    if (!params) params = {};
    this.navCtrl.setRoot(MusikerPage);
  }goToMusikerDetails(params){
    if (!params) params = {};
    this.navCtrl.setRoot(MusikerDetailsPage);
  }goToInstrumente(params){
    if (!params) params = {};
    this.navCtrl.setRoot(InstrumentePage);
  }goToInstrumentenDetails(params){
    if (!params) params = {};
    this.navCtrl.setRoot(InstrumentenDetailsPage);
  }goToKalender(params){
    if (!params) params = {};
    this.navCtrl.setRoot(KalenderPage);
  }
}
